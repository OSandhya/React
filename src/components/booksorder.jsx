...
class BooksOrder extends React.Component {
  ...
  componentDidMount() {
    axios
      .get("http://localhost:8080/api/viewOrderList")
      .then((response) => {
  ...
  handleDelete = (id) => {
    axios.delete(`http://localhost:8080/api/cancelOrder/${id}`).then((res) => {
  ...
  render() {
    return (
      <div className = "container">
        <Link to="/addorder/add" className="btn btn-secondary btn-large mt-3 float-end">
          Add
        </Link>
