...
class DamagedBooks extends React.Component {
  ...
  componentDidMount() {
    axios
      .get("http://localhost:8080/api/viewDamagedBooksList")
      .then((response) => {
  ...
  handleDelete = (id) => {
    axios.delete(`http://localhost:8080/api/deleteDamagedBooks/${id}`).then((res) => {
  ...
  render() {
    return (
      <div className = "container">
        <Link to="/api/viewDamagedBooksList/add" className="btn btn-secondary btn-large mt-3 float-end">
          Add
        </Link>
